...
The three layouts studied are shown in Fig.\ref{fig:tracking:layouts}. They have the same angular coverage, strip and pixel endcap geometry, and only differ in the pixel barrel sector.
...
The strip system is identical for the three layouts. It is composed by four barrel layers of identical length ($\pm 1350\ mm$), at radii 400 mm, 560 mm, 760 mm and 1000 mm. There
...
region $1.4 < |\eta| < 1.8$ the same happens for the innermost layers; the region $1.8 < |\eta| < 2.2$ is the transition region from barrel to endcap; the region $2.2 < |\eta| > 3.2$ is purely endcap
both in the pixel and in the strips, except for the two innermost barrel layers and there is the transition from pixel and strips to pixel only; the most forward region $3.2 < |\eta| < 4.0$ counts only on the innermost barrel layer and the pixel endcap disks.\\
...
geometrical one and emerges from the fact that particles with high pseudo-rapidity traverse the finite-depth flat sensors ionising the material in a broad z region. This effect is minimized in
the inclined layouts, where the inclined sensors ensures that the particle traverses a smaller amount of material. However, a sophisticated reconstruction algorithm could reduce the impact
of this effect in the ExtBrl4 layout by using the charge release distribution information. Unfortunately in the Step-1 layouts the reconstruction algorithm still needs to be optimized, so that
the results are to be taken carefully, especially when looking at the ExtBrl4 performances in the forward region.\\
