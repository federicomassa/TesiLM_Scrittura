...
\item to ensure good mass resolution, at least one of the muon candidates from the latter pair must lie in the
region $|\eta| < 2.7$. 
\end{itemize}
...
\subsubsection*{Background process generation}
...
\subsection{Event simulation and truth analysis}

The kinematics of the muons passing the above generation selection is shown in 
...
(referred as ExtBrl4, IExtBrl4 and InclBrl4). For the events in which the particles are within the detector acceptance, the distribution of
...
\subsubsection*{Identification of track candidates}
...
At this point, it has been verified that the number of events in which more than four tracks
pass all the cuts is below the 

If the number of tracks 
passing this selection is:
\begin{itemize}
\item \textbf{smaller} than four $\rightarrow$ the event is discarded;
\item \textbf{equal} to four $\rightarrow$ the final state is immediately reconstructible and the analysis cuts evaluated;
\item \textbf{larger} than four $\rightarrow$ a combinatorial process takes place.
\end{itemize}

The combinatorial problem is to identify, among combinations of four candidate tracks, the one that represent the Higgs candidate.
The selection of the \textit{best} combination proceeds as follows:
\begin{itemize}
\item Exclusion of the combinations of four tracks that do not form an electrically neutral system;
\item Exclusion of the combinations that do not pass the ordered $p_{T}$ cut, as in sec.\ref{subsec:physics:scoping};
\item From all the track pairs, identification of the one that form an electrically neutral system with invariant mass closest to the mass of the Z boson;
\item Among all combinations that contains this muon pair, selection of the one with highest invariant mass.
\end{itemize}



\subsubsection*{Physics analysis}
At this point, an event is either discarded or a combination of four tracks is identified. The remaining cuts are applied sequentially, with the
same order of sec.\ref{subsec:physics:scoping}. 

EFFICIENZE DI TAGLIO, ECC
...
as a function of the detector coverage. In Tab.(!!!!!!??????????) the value of the total Higgs and Z boson mass resolution divided
into the three regions is shown for the three ITk layouts considered. 
