...
\subsection{ATLAS Scoping-document analysis: inclusive $H \rightarrow ZZ^* \rightarrow 4\mu$}
...
As described in sec.\ref{subsec:PhysicsSimulation}, the first step of the fast simulaton 
...
each of which decaying into a neutral muon pair. This process was generated via
the Powheg Monte Carlo generator(see \ref{appendix:MC}). \\
...
is shown in Fig.\ref{fig:HZZ4mu:generatedMass}.

\begin{figure} [h]
	\centering
	\includegraphics[scale=0.3]{HZZ4mu/generatedMass}
	\caption{Higgs generated mass distribution.}
	\label{fig:HZZ4mu:generatedMass}
\end{figure}
...
(referred as ExtBrl4, IExtBrl4 and InclBrl4). 
...
will referred from now on as \textbf{hard-scatter truth} muons. \\

The next step was to find an appropriate \textbf{track matching} to every hard-scatter truth muon. Following the same
arguments described in sec.\ref{subsec:tracking:matching} for the particle gun analysis, a matching is found by looking,
for every hard-scatter truth muon, the track with minimum angular $\DeltaR$ distance from it. This is only done 
for the events in which all the hard-scatter truth muons are within the detector acceptance.
