...
\begin{figure} [h]
	\centering
	\includegraphics[width=.5\textwidth]{HiggsStrengthsTable}
...
around the hard-scattering particles. The value was chosen as it is the typical RoI width used by ATLAS (IN CHE TIPO DI STUDI?). A visual
...
\subsection{Fast simulation method applied to physics studies}\label{subsec:physicsSimulation}
...
The generation/simulation/digitization/reconstruction chain was run (NON CAPISCO CORREZIONE AL VERBO?) on a 
CERN LXPLUS machine. \\
...
Each module in this sector is formed by two pixel modules at a distance of 4 mm from each other. In the \textbf{Fully Inclined 4.0} (InclBrl4) layout,
...
The \textbf{SCT system} is identical for the three layouts. It is composed by four barrel layers of identical length ($\pm 1350\ mm$). The value of the radii are shown in tab. 
\ref{tab:tracking:barrelRadii}. There
...
with a 1 GeV requirement on the transverse momentum. In the employed sample there are X events in which more than one track is reconstructed and Y events with at least one hard secondary 
...
expected to have a small effect on the final result. The analysis selection applied is based on the Run-1 analysis:
...
producing events in selected kinematic regions. In particular, a minimum value of 12 GeV for the mass of
...
\end{tabular}}
	...
	previous step, while the total efficiency is the fraction of events passing all the cuts with respect to the total number of generated events. The uncertainty on the total efficiency is not shown for graphical 
...
\textbf{Total efficiency} & \color{dblue} 0.459 & \color{dblue} 0.249 & \color{dblue} 0.460 & \color{dblue} 0.249 & \color{dblue} 0.462 & \color{dblue} 0.248 \\ \hline 
	...
	fraction of events passing all the cuts with respect to the total number of generated events. The uncertainty on the total efficiency is not shown for graphical 
