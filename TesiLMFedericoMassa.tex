...
The Large Hadron Collider (LHC) accelerator is scheduled to be upgraded in two different phases: the first will begin after the end of Run 2 during the Long Shotdown 2 (LS2) in 2019-2020; the second will start at the end of Run 3 during the Long Shotdown 3 (LS3) in 2024-2026 (Fig.\ref{fig:HLLHC_Plan}). The latter upgrade phase will lead to what is referred as High-Luminosity LHC (HL-LHC) during which the
luminosity will reach the value of $7.5 \times 10^{34}\ cm^{-2}s^{-1}$, corresponding to approximately $3000\ fb^{-1}$ during ten years of data taking. \\
...
HL-LHC will allow to study new physics processes, such as the ones predicted by SUSY models and extra dimensions as well as improve Standard Model measurements, in particular Higgs boson 
couplings that are expected to be measured with a precision between 5\% and 30\%\cite{loi}. In fact, the increased statistics will allow rare channels to be measured,  $H \rightarrow \mu\mu$, vector boson 
fusion production of $H \rightarrow \gamma\gamma$ and $H \rightarrow \tau\tau$ and associated production $t\bar{t}H$ with $H \rightarrow \gamma\gamma$, as well as Higgs self couplings
...
lifetime, whereas the transition radiation tracker will reach 100\% occupancy at that luminosity. The Inner Detector will so be completely substituted by the Inner Tracker (ITk), which will
have to guarantee the same or an improved performance in the harsh conditions of HL-LHC while maximizing the acceptance. At the design luminosity, the expected number of in-time pileup events will be $<\mu > = 200$, much
higher than that expected at the end of Run 3 ($<\mu > = 80$) and even more with respect to the current one ($<\mu > = 35$??), introducing new challenges in the tracking. The increase of the number
